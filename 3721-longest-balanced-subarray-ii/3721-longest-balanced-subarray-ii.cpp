struct LazySegTree {
    ...
    void pull(int node) {
        tree[node].minVal =
            min(tree[node * 2].minVal, tree[node * 2 + 1].minVal);
        tree[node].maxVal =
            max(tree[node * 2].maxVal, tree[node * 2 + 1].maxVal);
    }
    ...
    int findRightmostZero(int node, int left, int right) {
        if (tree[node].minVal > 0 || tree[node].maxVal < 0)
            return -1;
        if (left == right)
            return left;
        push(node);
    ...
    int findRightmostZero() { return findRightmostZero(1, 0, size - 1); }
};
    ...
    int longestBalanced(vector<int>& nums) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            if (lastIndex.count(nums[i]))
                nextOccurrence[i] = lastIndex[nums[i]];
            lastIndex[nums[i]] = i;
        }
